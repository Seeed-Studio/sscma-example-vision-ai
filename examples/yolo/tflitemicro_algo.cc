...
extern "C" int tflitemicro_algo_run(uint32_t img, uint32_t ow, uint32_t oh)
{
    ...
    {
        obj.x = uint16_t(float(obj.x) / float(w) * float(ow));
        obj.y = uint16_t(float(obj.y) / float(h) * float(oh));
        obj.w = uint16_t(float(obj.w) / float(w) * float(ow));
        obj.h = uint16_t(float(obj.h) / float(h) * float(oh));
    }
