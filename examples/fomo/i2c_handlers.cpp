...
//     hx_lib i2ccomm_enable_write( i2c_write_buf);
//     hx_lib i2ccomm_enable_read( i2c_read_buf, I2CCOMM_MAX_RBUF_SIZE);
...
//          _get_det_result_data(index, &obj);

//         //  _printf("x:%d y:%d w:%d h:%d confidence:%d target:%d, index:%d\n", obj.x, obj.y, obj.w, obj.h, obj.confidence, obj.target, index);
...
//          _get_cnt_result_data(index, &obj);
//         *len = sizeof(object_counting_t);
//         //  _printf("count:%d target:%d, index:%d\n", obj.count, obj.target, index);
//         memcpy(write_buf, &obj, *len);
//     }
...
//     uint16_t index = 0;
//     // get result size
//     uint16_t size =  _get_cnt_result_len();
...
//     // get available size
//     uint16_t available_size = (max_length - sizeof(COUNTING_PREVIEW_FORMATE)) / (COUNTING_PR VEW_ELEMENT_SIZE * COUNTING_PR VEW_ELEMENT_NUM);
...
//     // element
//     char element[COUNTING_PR VEW_ELEMENT_NUM][COUNTING_PREVIEW_MAX_SIZE * COUNTING_PR VEW_ELEMENT_SIZE] = {0};
...
//     uint16_t index = 0;
//     // get result size
//     uint16_t size =  _get_det_result_len();
...
//     // get available size
//     uint16_t available_size = (max_length - sizeof(DETECTION_PREVIEW_FORMATE)) / (DETECTION_PR VEW_ELEMENT_SIZE * DETECTION_PR VEW_ELEMENT_NUM);
...
//     // element
//     char element[DETECTION_PR VEW_ELEMENT_NUM][DETECTION_PREVIEW_MAX_SIZE * DETECTION_PR VEW_ELEMENT_SIZE] = {0};
...
//          _start_impulse(false, false, false, _algoConfig.confidence / 100.0f);
...
//             }
//             _algoConfig.ret_len =  _get_det_result_len();
//         }
//         else if (_algoConfig.algo == ALGO_OBJECT_COUNTING)
//         {
...
//                 webusb_write_text(_preview_buf, strlen(_preview_buf));
//             }
//             _algoConfig.ret_len =  _get_cnt_result_len();
//         }
...
// I2CServer *  i2c_init( DeviceVisionAI *device)
// {
...
//     hx_lib i2ccomm_init(i2c_cfg);

//     hx_lib i2ccomm_enable_write( i2c_write_buf);
//     hx_lib i2ccomm_start( i2c_read_buf, I2CCOMM_MAX_RBUF_SIZE);
