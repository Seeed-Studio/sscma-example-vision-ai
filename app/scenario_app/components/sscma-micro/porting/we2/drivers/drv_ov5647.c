...
void set_mipi_csirx_enable() {
    ...
    EL_LOGD("MIPI BITRATE 1LANE: %dM\n", bitrate_1lane);
    EL_LOGD("MIPI DATA LANE: %d\n", mipi_lnno);
    EL_LOGD("MIPI PIXEL DEPTH: %d\n", pixel_dpp);
    EL_LOGD("MIPI LINE LENGTH: %d\n", line_length);
    EL_LOGD("MIPI FRAME LENGTH: %d\n", frame_length);
    EL_LOGD("MIPI CONTINUOUSOUT: %d\n", continuousout);
    EL_LOGD("MIPI DESKEW: %d\n", deskew_en);
    ...
    EL_LOGD("MIPI RX FIFO FILL: %d\n", rx_fifo_fill);
    EL_LOGD("MIPI TX FIFO FILL: %d\n", tx_fifo_fill);
...
el_err_code_t drv_ov5647_init(uint16_t width, uint16_t height) {
    ...
    EL_LOGD("mclk DIV3, xshutdown_pin=%d\n", DEAULT_XHSUTDOWN_PIN);
    ...
    EL_LOGD("hx_drv_cis_set_slaveID(0x%02X)\n", CIS_I2C_ID);
    ...
    EL_LOGD("start_x: %d start_y: %d width: %d height: %d\n", start_x, start_y, width, height);
    ...
    _wdma1_baseaddr = (uint32_t)el_malloc(width * height / 4);  // JPEG
    ...
    _wdma3_baseaddr = (uint32_t)el_malloc(width * height * 3 / 2);
