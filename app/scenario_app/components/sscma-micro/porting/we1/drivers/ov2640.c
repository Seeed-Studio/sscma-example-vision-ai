...
static int8_t ov2640_get_sensor_id(uint32_t* sensor_id) {
    ...
    if (hx_drv_cis_get_reg(reg_addr, &reg_value) != HX_CIS_NO_ERROR) {
        EL_LOGW("<%s><%d>get sensor id failed\n", __func__, __LINE__);
        return -EL_EIO;
    }
...
static int8_t ov2640_set_output_size(uint16_t width, uint16_t height) {
    ...
    uint8_t  temp;
    if (width % 4 || height % 4) {
        EL_LOGW("camera size param error\n");
        return -EL_EINVAL;
    }
    if (width > OV2640_MAX_WIDTH || height > OV2640_MAX_HEIGHT) {
        EL_LOGW("camera size overflow error\n");
        return -EL_EINVAL;
    }
    ...
    if (width <= 64 && height <= 64) {
        ...
        EL_LOGI("CIF: width = %d, height = %d, outw = %d, outh = %d\n", width, height, outw, outh);
        hx_drv_cis_setRegTable(sensor_ov2640_cif_setting,
                               sizeof(sensor_ov2640_cif_setting) / sizeof(sensor_ov2640_cif_setting[0]));
        ...
        outh = 480;
        outw = 640;
        // EL_LOGI("SVGA: width = %d, height = %d, outw = %d, outh = %d\n", width, height, outw, outh);
        hx_drv_cis_setRegTable(sensor_ov2640_svga_setting,
                               sizeof(sensor_ov2640_svga_setting) / sizeof(sensor_ov2640_svga_setting[0]));
        /* code */
    } else {
        // UXGA
        outh = height;
        outw = width;
        // EL_LOGI("UXGA: width = %d, height = %d, outw = %d, outh = %d\n", width, height, outw, outh);
        hx_drv_cis_setRegTable(sensor_ov2640_uxga_setting,
                               sizeof(sensor_ov2640_uxga_setting) / sizeof(sensor_ov2640_uxga_setting[0]));
    }
...
Camera_Hal_Struct ov2640_driver = {
  ...
  .sensor_cfg_len     = HX_CIS_SIZE_N(sensor_ov2640_setting, HX_CIS_SensorSetting_t),
